    ...
    def upload_everything(self):
        ...
        for device_id, values in self.power_values.items():
            for value in values:
                ts = value[0]
                val = value[1]
                action = self._create_action_payload(device_id, val, "set_power_level", self.is_initial_event(ts), "W",
                                                     device_id)
        ...
        for device_id, values in self.energy_price_values.items():
            for value in values:
                ts = value[0]
                val = value[1]
                action = self._create_action_payload(device_id, val, "new_electricity_price", self.is_initial_event(ts),
                                                     "$/kWh", device_id)
                actions.append({"timestamp": ts, "action": action})
        ...
        for device_id, values in self.fuel_level_values.items():
            for value in values:
                ts = value[0]
                val = value[1]
                action = self._create_action_payload(device_id, val, "fuel_level", self.is_initial_event(ts), "%",
                                                     device_id)
                actions.append({"timestamp": ts, "action": action})
