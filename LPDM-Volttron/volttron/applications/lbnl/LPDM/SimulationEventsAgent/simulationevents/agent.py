...
class SimulationEventsAgent(SimulationAgent):
    def __init__(self, config_path, **kwargs):
        raise RuntimeError(
            "This class probably does not work with latest changes.  It is not used by the CBERD experiment (currently) so is being left broken due to time constraints.")
    ...
    def send_events(self, events_to_send):
        for event in events_to_send:
            ...
            self.vip.pubsub.publish("pubsub",
                                    CHANGE_DEVICE_PARAMETER_TOPIC_SPECIFIC_AGENT.format(id=event["device_id"]), headers,
                                    message)
