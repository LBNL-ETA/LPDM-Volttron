...
from applications.lbnl.SmapInterfaceAgent.smapinterface.topics import *
...
class TROPEC_DashboardLoggingAgent(PublishMixin, BaseAgent):
    def __init__(self, **kwargs):
        super(TROPEC_DashboardLoggingAgent, self).__init__(**kwargs)      
    ...
    def on_TROPEC_power_message(self, peer, sender, bus, topic, headers, message):
    ...
    def on_TROPEC_energy_price_message(self, peer, sender, bus, topic, headers, message):
        from_id = headers[headers_mod.FROM]
        timestamp = float(headers["timestamp"])
    ...
    def on_TROPEC_fuel_level_message(self, peer, sender, bus, topic, headers, message):
        from_id = headers[headers_mod.FROM]
        timestamp = float(headers["timestamp"])
    ...
    #@match_all
    @PubSub.subscribe("pubsub", TROPEC_BASE)
...
def main(argv=sys.argv):
    ...
    utils.vip_main(TROPEC_DashboardLoggingAgent)
