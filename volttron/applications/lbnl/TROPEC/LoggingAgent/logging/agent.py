...
class TROPEC_LoggingAgent(PublishMixin, BaseAgent):
    ...
    def upload_everything(self):
        ...
        for device_id, values in self.power_values.items():
            ...
            self.vip.pubsub.publish("pubsub", UPLOAD_REQUEST_TOPIC + "/TROPEC_Logging", headers, msg)
...
#         self.publish_json(UPLOAD_REQUEST_TOPIC + "/TROPEC_Logging", headers, msg)
    ...
    def on_TROPEC_fuel_level_message(self, topic, headers, message, matched):
        message = jsonapi.loads(message[0])
        from_id = headers[headers_mod.FROM]
        timestamp = float(headers["timestamp"])
...
#                    "SmapRoot" : "http://elnhv.lbl.gov/",
#                    #'Stream' : "/simulation/{id}".format(id = from_id),
#                    'Stream' : "{scenario_id}/{id}/fuel_level".format(scenario_id = self.scenario_id, id = from_id),
#                    'ApiKey' : "3EYQy04hlPpA03SixKcRJaIreUrnpdYu9bNn",
#                    'StreamUUID' : "USE EXISTING"                      
#                    }
#         metadata = {"UnitofMeasure" : "g", "Timezone" : "America/Los_Angeles"}
#         
#         msg = {"timeseries" : [[jan_1_2015 + int(timestamp * 1000), fuel_level]], "metadata" : metadata}                           
#         
#         self.publish_json(UPLOAD_REQUEST_TOPIC + "/TROPEC_Logging", headers, msg)
...
def main(argv=sys.argv):
    ...
    utils.vip_main(TROPEC_LoggingAgent)
